...
from info import vs30_shapefile_filename,bedrock_vs30,distance_type,final_result_filename2
from info import Cat_Fault,Vs30_Polygons
from DIST import Zone_Based_Distance,Find_R_Zones, Fault_Proj
from RISK_ import Find_Vs30, GMA_DAMAGE,NEHRP_Class
from BUFF import Color_Buffer
...
#### CALCULATE R·······························································
    ...
    with open(os.path.join(outputpath,final_result_filename2),'w') as w: 
...
###         gma_and_damage_array contains: ['PGA','SA_te','P(nul)','P(low)','P(mod)','P(ext)','P(com)','D(medio)','D(85%)','V_Debris']
            dato=OrderedDict([('UID',uid),\
                  ...
                  ('logPGA', round(float(gma_and_damage_array[10]), 3)),\
                  ('IntAK07', round(float(gma_and_damage_array[11]), 3)),\
                  ('IntM10', round(float(gma_and_damage_array[12]), 3)),\
                  ('Int', round(float(gma_and_damage_array[13]), 3)),\
                  ('IntM', round(float(gma_and_damage_array[14]), 3)),\
    ...
    print('Elapsed time before debris estimations: %s'%(datetime.datetime.now()-time)) 
    Color_Buffer()
    print('Total elapsed time %s'%(datetime.datetime.now()-time)) 
