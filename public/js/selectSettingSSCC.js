    ...
    fault = new L.geoJSON(fault_mini,{
        ...
        onEachFeature: function(feature, layer){
            ...
            layer.on('click', function(e){
                ...
                // Guardar parámetros del terremoto seleccionado
                window.lastEarthquakeParams = {
                    lat: LocationY,
                    lng: LocationX,
                    Mag: Magnit,
                    Strike: Strike,
                    Dip: Dip,
                    FM: SenseOfMov.toString(),
                    Length: Length,
                    Ztop: Ztop,
                    Width: Width,
                    Id: idText,
                    ...
                    caseValue: window.casoValue || casoValue
                };
           ...
           fault.eachLayer(function(m) { 
              if (circleBound.contains(m.getBounds().getCenter())) {
                v = L.geoJSON(m.feature,{
                    ...
                    // CORRECCIÓN: actualizar lastEarthquakeParams (igual que en el
                    // handler del mapa principal) para que showTextWarning use
                    // los parámetros de esta falla, no los de la última clicada en el mapa.
                    window.lastEarthquakeParams = {
                        lat: LocationY,
                        lng: LocationX,
                        Mag: Magnit,
...
$('#end').on('click', function(){
    ...
    rangeSliderDip.slider({disabled:true});
    
    joinLayer.clearLayers();
    
    mapActive = false;
    
    rangeSlider.slider({value: 45});
     $('.createSlider label span').html(rangeSlider.slider('value'));
    rangeSliderDip.slider({value: 45});
     $('.createSlider-dip label span').html(rangeSliderDip.slider('value'));//$('#slider').slider('value')
			...
			$.ajax('/createData', {
                ...
                success: function (data) {
					if(data){
                       ...
                       var iframeMap = window.parent.document.getElementById('iframeMap');
                       if (iframeMap) {
                           try {
                               // Reasignar src con timestamp fuerza descarga completa sin caché,
                               // a diferencia de reload(true) que los navegadores modernos ignoran.
                               var baseSrc = iframeMap.src.split('?')[0];
                               iframeMap.src = baseSrc + '?v=' + Date.now();
                               window.parent.location.hash = '#mapShow';
                           } catch (reloadErr) {
                               console.warn('No se pudo recargar el iframe del mapa:', reloadErr);
                           }
                       }
                    }
                     console.log(data);
...
function showTextWarning(t){
    ...
    if(t == 'automatic'){
        ...
        clickOK.click(function(){
            ...
            // CORRECCIÓN: leer casoValue ahora (al confirmar), no cuando se clicó la falla,
            // para que un cambio de escenario posterior a la selección quede reflejado.
            var currentCaseValue = (window.casoValue !== undefined) ? window.casoValue : casoValue;
            if (window.lastEarthquakeParams) {
                window.lastEarthquakeParams.caseValue = currentCaseValue;
                createFile(buffer, currentCaseValue, window.lastEarthquakeParams.lat, window.lastEarthquakeParams.lng, window.lastEarthquakeParams.Mag, window.lastEarthquakeParams.Strike, window.lastEarthquakeParams.Dip, window.lastEarthquakeParams.FM, window.lastEarthquakeParams.Length, window.lastEarthquakeParams.Ztop, window.lastEarthquakeParams.Width, window.lastEarthquakeParams.Id, window.lastEarthquakeParams.FN, window.lastEarthquakeParams.SN, window.lastEarthquakeParams.Area, window.lastEarthquakeParams.NetSlipRat);
            } else {
                createFile(buffer, currentCaseValue, LocationY, LocationX, Magnit, Strike, Dip, SenseOfMov.toString(), Length, Ztop, Width, idText, FaultName, SegmentNam, Area, NetSlipRat);
            }
        }); // Cierra clickOK.click
        ...
        clickOK.click(function(){
            ...
            $("#warnigOption", window.parent.document).css('display','none');
            $('a#viewBuffer', window.parent.document).css('display','none');
            createFile(buffer,casoValue,LocationXC,LocationYC,MagnitC,StrikeC,DipC,SenseOfMovC,LengthC,ZtopC,WidthC);
        });
        }
